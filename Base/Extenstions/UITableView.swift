...
public extension UITableView {
    ...
    public func dequeueReusableCell<T>(withType type:T.Type) -> T? {
        let name = "\(T.self)"
        return dequeueReusableCell(withIdentifier: name) as? T
    }
    
    public func dequeueReusableCell<T>(withType type:T.Type) -> T? where T: HasDisposeBag {
        let name = "\(T.self)"
    ...
    public func dequeueReusableCell<T>(withType type:T.Type, for indexPath:IndexPath) -> T? {
        
        let name = "\(T.self)"
        return dequeueReusableCell(withIdentifier: name, for: indexPath) as? T
    }
    
    public func dequeueReusableCell<T>(withType type:T.Type, for indexPath:IndexPath) -> T? where T: HasDisposeBag {
